...
void USmartBunkerSelectionSubsystem::GetTopK(const AActor* Viewer, int32 K, TArray<FBunkerCandidate>& OutTopK) const
{
	...
	BuildCandidateSet(Viewer, 32, Cands);
	if (Cands.Num() == 0 || K <= 0)
	{
		OutTopK.Reset();
		return;
	...
	// Normalize distance (near is better) using DebugMaxDistance
	for (FBunkerCandidate& C : Cands)
	{
		const float NormDist = FMath::Clamp(C.Distance / FMath::Max(10.f, DebugMaxDistance), 0.f, 1.f);
		const float DistTerm = 1.f - Sigmoid01(NormDist, /*k=*/8.f, /*c=*/0.45f); // near → ~1, far → ~0
		const float AngleTerm = (C.CosLaneAngle * 0.5f) + 0.5f; // [-1,1] → [0,1], facing you is nicer
	...
	if (K > Cands.Num()) K = Cands.Num();
	Algo::Sort(Cands, [](const FBunkerCandidate& A, const FBunkerCandidate& B)
	{
		return A.Score > B.Score;
	});
	if (K < Cands.Num())
	{
		Cands.SetNum(K, /*bAllowShrinking=*/false);
	}
	OutTopK = TArray<FBunkerCandidate>(Cands.GetData(), K);
}
...
void USmartBunkerSelectionSubsystem::Tick(float DeltaTime)
{
	...
	// Simple local debug: show Top-K for first local player's pawn
	...
	for (int32 i = 0; i < Top.Num(); ++i)
	{
		...
		// Text with score
		const FString Txt = FString::Printf(TEXT("Top[%d] S=%.2f  d=%.0f  cosθ=%.2f  slot=%d"),
			i, C.Score, C.Distance, C.CosLaneAngle, C.SlotIndex);
