...
void USmartBunkerSelectionSubsystem::DebugDrawTopK(APawn* ViewerPawn)
{
	...
	TArray<FBunkerCandidateDepricated> Top;
	GetTopKAcrossBunkers(ViewerPawn, DebugTopK, Top);

	for (int32 i = 0; i < Top.Num(); ++i)
	{
		const FBunkerCandidateDepricated& C = Top[i];
...
void USmartBunkerSelectionSubsystem::BuildCandidates(const AActor* Viewer, int32 MaxCandidates, TArray<FBunkerCandidateDepricated>& OutCandidates) const
{
//	OutCandidates.Reset();
//	if (!Viewer) return;
//
...
void USmartBunkerSelectionSubsystem::GetTopKAcrossBunkers(const AActor* Viewer, int32 K, TArray<FBunkerCandidateDepricated>& OutTopK) const
{
...
void USmartBunkerSelectionSubsystem::GetTopKWithinBunker(const AActor* Viewer, ABunkerBase* Bunker, int32 K, TArray<FBunkerCandidateDepricated>& OutTopK) const
{
//	OutTopK.Reset();
...
//	for (int32 i = 0; i < SlotCount; ++i)
//	{
//		//if (Bunker->IsSlotOccupie(i)) continue; // only free slots
